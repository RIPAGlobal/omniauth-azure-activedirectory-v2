...
You can do something like this if specify a static / fixed configuration:
...
In your `config/initializers/devise.rb` file you can do something like this if specify a static / fixed configuration:
...
All of the items listed below are optional, unless noted otherwise. They can be provided either in the static options Hash as shown in examples above, or via *read accessor instance methods* in a provider class (more on this later).
...
In addition, as a special case, if the request URL contains a query parameter `prompt`, then this will be written into `authorize_params` under that key, overwriting if present any other value there. Note that this comes from the current request URL at the time OAuth flow is commencing, _not_ via static options Hash data or via a custom provider class - but you _could_ just as easily set `scope` inside a custom `authorize_params` returned from a provider class; the request URL query mechanism is just another way of doing the same thing.

#### Explaining `authorize_params`

This is where we get to explain `authorize_params` a bit more. Key-value pairs in here are transformed into a query string and added to existing standard OAuth query data in the initial redirection from your web site to the Microsoft Azure AD login page at the start of OAuth flow. You can find these listed some way down the table just below an OAuth URL example at:
